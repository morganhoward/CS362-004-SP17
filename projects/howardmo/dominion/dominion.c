...
// refactor of adventurer card effect
/*
 * Effect: Reveal cards from your deck until you reveal 2 Treasure cards.
 *         Put those Treasure cards into your hand and discard the other revealed cards.
 */
int card_adventurer(struct gameState *state, int currentPlayer, int handPos) {
    // declare vars
    int drawn_treasure = 0;
    int cardDrawn = 0;
    int temphand[MAX_HAND];
    int temphand_count = 0;
    ...
    while (drawn_treasure < 2) {
        ...
        // top card of hand is the most recently drawn card.
        cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer] - 1];

        if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold) {
            drawn_treasure++;
        } else {
            temphand[temphand_count] = cardDrawn;
            state->handCount[currentPlayer]--;
            temphand_count++;
        }
    }

    while (temphand_count - 1 >= 0) {
        // discard all cards in play that have been drawn
        state->discard[currentPlayer][state->discardCount[currentPlayer]++] = temphand[temphand_count - 1];
        temphand_count--;
    }

    // BUG: adventurer card is not discarded
    // BUGFIX: discard adventurer card
    discardCard(handPos, currentPlayer, state, 0);

    return 0;
}

// refactor of smithy card effect
/*
 * Effect: +3 Cards
 */
int card_smithy(struct gameState *state, int currentPlayer, int handPos) {
    ...
    // BUGFIX: change 4 to a 3
    for (int i = 0; i < 3; i++)
        //for (i = 0; i < 4; i++)  // BUGGED version
    {
        drawCard(currentPlayer, state);
    }
...
// refactor of great hall card effect
/*
 * Effect: +1 card
 *         +1 action
 */
int card_great_hall(struct gameState *state, int currentPlayer, int handPos) {
    // +1 card
    drawCard(currentPlayer, state);
...
// refactor of village card effect
/*
 * Effect: +1 card
 *         +2 actions
 */
int card_village(struct gameState *state, int currentPlayer, int handPos) {
    ...
    // BUGFIX: change 3 to a 2
    state->numActions = state->numActions + 2;
    //state->numActions = state->numActions + 3;  // BUGGED version
...
// refactor of tribute card effect
/*
 * Effect: The player to your left reveals then discards the top 2 cards of his deck.
 *         For each differently named card revealed, if it is an ...:
 *             Action card: +2 actions
 *             Treasure card: +2 coins
 *             Victory card: +2 cards
 */
int card_tribute(struct gameState *state, int currentPlayer, int nextPlayer) {
    // vars
    int tributeRevealedCards[2] = {-1, -1};
    int i = 0;

    // have player to the left draw 2 cards
    for (i = 0; i < 2; i++) {
        drawCard(nextPlayer, state);
        // top card of hand is the most recently drawn card.
        tributeRevealedCards[i] = state->hand[nextPlayer][state->handCount[nextPlayer] - 1];
        discardCard(state->handCount[nextPlayer] - 1, nextPlayer, state, 0);
    }

    // compare the two cards to determine if they are different
    if (tributeRevealedCards[0] == tributeRevealedCards[1]) {
        // drop the second card because it's a duplicate
        state->playedCards[state->playedCardCount] = tributeRevealedCards[1];
        state->playedCardCount++;
        tributeRevealedCards[1] = -1;
    }

    // award card benefits based on number of different cards
    // BUG: cards with multiple types aren't awarded benefits for each type (e.g. Great Hall, action-victory)
    for (i = 0; i <= 2; i++) {
        // treasure card
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus) {
    ...
    switch (card) {
        case adventurer:
            return card_adventurer(state, currentPlayer, handPos);
        ...
        case smithy:
            return card_smithy(state, currentPlayer, handPos);

        case village:
            return card_village(state, currentPlayer, handPos);
        ...
        case great_hall:
            return card_great_hall(state, currentPlayer, handPos);
        ...
        case tribute:
            return card_tribute(state, currentPlayer, nextPlayer);
